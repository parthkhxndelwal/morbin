...
/**
 * Light tunnel background — WebGL fiber-optic tunnel with light pulses.
 * Ported from React Bits (MIT License, https://reactbits.dev — by DavidHDev),
...
type VecUniform = { value: Float32Array };
...
function setNum(program: Program, key: string, v: number) {
  (program.uniforms[key] as NumUniform).value = v;
}

function setVec(program: Program, key: string, x: number, y: number, z?: number) {
  const arr = (program.uniforms[key] as VecUniform).value;
  arr[0] = x;
  arr[1] = y;
  if (z !== undefined && arr.length > 2) arr[2] = z;
}
...
export function LightTunnel({
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uOpacity: { value: 1.0 },
        uCableColor: { value: new Float32Array([0.658, 0.333, 0.969]) },
        uPulseColor: { value: new Float32Array([0.658, 0.333, 0.969]) },
        uTunnelColor: { value: new Float32Array([0.322, 0.153, 1]) },
    ...
    const setSize = () => {
      ...
      const res = (program.uniforms.iResolution as VecUniform).value;
    ...
    const loop = (t: number) => {
      (program.uniforms.iTime as NumUniform).value = (t - t0) * 0.001;
      ...
      const off = (program.uniforms.uMouseOffset as VecUniform).value;
  ...
  useEffect(() => {
    ...
    setNum(program, "uSpeed", speed);
    setNum(program, "uFlowDir", flowDirection === "outward" ? -1.0 : 1.0);
    setNum(program, "uPulseSpeed", pulseSpeed);
    setNum(program, "uPulseLength", pulseLength);
    setNum(program, "uPulseBlend", pulseBlend);
    setNum(program, "uPulseWidth", pulseWidth);
    setNum(program, "uCableCount", cableCount);
    setNum(program, "uThickness", thickness);
    setNum(program, "uRimWidth", rimWidth);
    setNum(program, "uWaviness", waviness);
    setNum(program, "uSway", sway);
    setNum(program, "uSize", size);
    setVec(program, "uCenter", centerX, centerY);
    setNum(program, "uGlow", glow);
    setNum(program, "uFadeNear", fadeNear);
    setNum(program, "uFadeFar", fadeFar);
    setNum(program, "uBrightness", brightness);
    setNum(program, "uColorVariance", colorVariance ? 1.0 : 0.0);
    setNum(program, "uGrain", grain ? 1.0 : 0.0);
    setNum(program, "uGrainIntensity", grainIntensity);
    setNum(program, "uOpacity", opacity);
    setNum(program, "uLightMode", lightMode ? 1.0 : 0.0);
    const [cr, cg, cb] = hexToRgb(cableColor);
    setVec(program, "uCableColor", cr, cg, cb);
    const [pr, pg, pb] = hexToRgb(pulseColor);
    setVec(program, "uPulseColor", pr, pg, pb);
    const [tr, tg, tb] = hexToRgb(tunnelColor);
    setVec(program, "uTunnelColor", tr, tg, tb);
    setNum(program, "uTunnelOpacity", tunnelOpacity);
  }, [
    cableColor,
    pulseColor,
    tunnelColor,
